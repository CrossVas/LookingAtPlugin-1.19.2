        ...
        if (canHandle(player)) {
            if (blockEntity instanceof BaseTileEntity baseTile) {
                if (baseTile instanceof CreativeSourceTileEntity) {
                    ...
                    helper.addEnergyElement(storage.getStoredEU(), storage.getMaxEU(), Component.translatable("ic2.probe.eu.storage.full.name", storage.getStoredEU(), storage.getMaxEU()).withStyle(ChatFormatting.WHITE));
                }
            }
        }
    }
